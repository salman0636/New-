...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (mouseX - (120 * (BeadCoords.x)) <= 60 && mouseY - (120 * (BeadCoords.y)) <= 60 && BeadCoords.x >= 0 && BeadCoords.x <= 4 && BeadCoords.y >= 0 && BeadCoords.y <= 4 && board[BeadCoords.y][BeadCoords.x] == result && menu == false && reading == false && tossdone != false && won == false) {
                    ...
                    isValid = Game.valid(board, selectedBeadCoords.x, selectedBeadCoords.y, result);
                }
                if (mouseX - (120 * (BeadCoords.x)) <= 60 && mouseY - (120 * (BeadCoords.y)) <= 60 && BeadCoords.x >= 0 && BeadCoords.x <= 4 && BeadCoords.y >= 0 && BeadCoords.y <= 4 && board[BeadCoords.y][BeadCoords.x] == 0 && selectedBeadCoords.x != -1 && selectedBeadCoords.y != -1 && menu == false && reading == false && tossdone != false && won == false) {
                    ...
                    cout << "\nFINAL COORDINATES\n";
                    cout << finalY << ", " << finalX;
                    isValid = Game.check(board, finalY, finalX, selectedBeadCoords.y, selectedBeadCoords.x, result);
                    if (isValid == false) {
                        if (result == 1) {
        ...
        if (menu == true) {
            for (int i = 0; i < Game.maxShapes; ++i) {
                window.draw(Game.shapes[i].shape);
            }
        }
        if (menu == false) {
            Game.drawLines(window);
            Game.drawBoard(window);
            if (selectedBeadCoords.y != -1 && selectedBeadCoords.x != -1)
                Game.possibilities(board, selectedBeadCoords.y, selectedBeadCoords.x, result, window);
        }
        if (moving == true)
        ...
        if (menu == false)
            ...
            Game.prntmenu(window);
        if (reading == true)
            Game.displayreading(window);
